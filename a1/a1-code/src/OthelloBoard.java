...
public class OthelloBoard {
	...
	public int getCount(char player) {
		...
		for (int row = 0; row < this.size; row++) {
			for (int col = 0; col < this.size; col++) {
				if ((player == P1 && this.board[row][col] == P1) || (player == P2 && this.board[row][col] == P2)) {
					cnt++;
				}
			}
		}
	...
	private char checkForPattern(int row, int col, int drow, int dcol) {
		...
		ArrayList<Character> seq = getPlayers(row, col, drow, dcol);
	...
	/**
	 * Get all the player tokens starting from (row, col) with the directions
	 * drow, dcol.
	 * 
	 * @param row  the row to begin collecting from
	 * @param col  the column to begin collecting from
	 * @param drow the row direction which is either -1 (up on the board), 
	 * 				1 (down on the board), or 0 (no change in row)
	 * @param dcol the col direction which is either -1 (left on the board), 
	 * 				1 (right on the board), or 0 (no change in column)
	 * @return the ArrayList of all player tokens
	 */
	private ArrayList<Character> getPlayers(int row, int col, int drow, int dcol){
		int x = row;
		int y = col;
		ArrayList<Character> seq = new ArrayList<>();
		while (getToken(x, y) != EMPTY) {
			char player = getToken(x, y);
			seq.add(player);
			x = x + drow;
			y = y + dcol;
		}
	...
	private int flip(int row, int col, int drow, int dcol, char player) {
		...
		if (checkForPattern(row, col, drow, dcol) == player) {
			while (getToken(x, y) != EMPTY && getToken(x, y) != player) {
		...
		if (checkForPattern(row, col, drow, dcol) == otherPlayer(player)) {
			return 0;
		}
		ArrayList<Character> seq = getPlayers(row, col, drow, dcol);
		if (! seq.contains(P1) || ! seq.contains(P2)) {
			return -1;
		}
	...
	public boolean move(int row, int col, char player) {
		if (this.board[row][col] != EMPTY || ! validCoordinate(row, col)) {
			return false;
		}
		boolean temp = false;
