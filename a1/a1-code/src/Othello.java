...
public class Othello {
	...
	/**
	 * Returns the next player if he has valid moves.
	 * 
	 * @param character representing player P1 or P2
	 * @return the next player who can has a move
	 */
	public char nextPlayer(char player) {
		if (board.hasMove() != OthelloBoard.EMPTY) {
			char next = OthelloBoard.otherPlayer(player);
			return next;
		}
		else {
			char end = OthelloBoard.EMPTY;
			return end;
		}
	}
	...
	public boolean move(int row, int col) {
		boolean temp = board.move(row, col, this.whoseTurn);
		if (temp) {
			this.numMoves++;
			this.whoseTurn = nextPlayer(this.whoseTurn);
			return true;
		}
		else {
			this.whoseTurn = nextPlayer(this.whoseTurn);
	...
	public char getWinner() {
		...
		int p_1 = getCount(OthelloBoard.P1);
		int p_2 = getCount(OthelloBoard.P2);
		if (p_1 > p_2) {
